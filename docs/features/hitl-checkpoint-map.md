...
> **Architectural Pivot (2026-01-19)**: Phase 0 was simplified to Quick Start - no HITL. The first checkpoint is now `approve_discovery_output` in Phase 1. See [ADR-006](../adr/006-quick-start-architecture.md).
...
|-------|------------|---------|---------|---------|
| 0 | ~~`approve_founders_brief`~~ | ~~Brief compiled~~ | ~~approve, revise, reject~~ | **REMOVED** |
| 1 | `approve_discovery_output` | Brief + VPC complete | approve, edit, reject | approve (if fit ≥ 70) |
...
**Total**: 10 HITL checkpoints across Phases 1-4 (7 unique, 3 conditional in Phase 2). Phase 0 has no HITL.
...
> **Architectural Pivot (2026-01-19)**: Phase 0 was simplified to Quick Start. There are no HITL checkpoints in Phase 0.

Phase 0 is now a simple form submission. The first HITL checkpoint is `approve_discovery_output` in Phase 1.

### Deprecated: `approve_founders_brief`

This checkpoint was **removed** by the Quick Start pivot. The Founder's Brief is now generated by Phase 1 BriefGenerationCrew and reviewed as part of `approve_discovery_output`.
...
**Trigger Location**: `src/modal_app/phases/phase_1.py` (after BriefGenerationCrew and discovery crews complete)

**Trigger Condition**: After BriefGenerationCrew generates Brief and FitAssessmentCrew scores VPC fit

> **Note**: This checkpoint combines the former `approve_founders_brief` (Phase 0) and `approve_vpc_completion` (Phase 1) into a single review point.

| Option | Value | Description | Next Action |
|--------|-------|-------------|-------------|
| **Approve** | `approve` | Brief and VPC are accurate | → Continue Phase 1 experiments |
| Edit | `edit` | User edited Brief before approving | → Continue Phase 1 |
| Reject | `reject` | Research is fundamentally wrong | → Return to Quick Start |

**Default Recommendation**: `approve` if `fit_score >= 70`, else `edit`

**Gate Condition**: `fit_score >= 70` required to proceed to Phase 2

**Context Payload**:
```json
{
  "raw_idea": "User's original Quick Start input",
  "founders_brief": { /* AI-generated FoundersBrief model */ },
