...
class LandingPageDeploymentTool(BaseTool):
    ...
    def _deploy_with_httpx(
        ...
        with httpx.Client(timeout=60.0) as client:
            ...
            # Step 2: Deploy using ZIP method (simpler than digest-based upload)
            # Creates ZIP in memory and uploads in single POST request
            import zipfile
            import io
            ...
            if deploy_response.status_code not in [200, 201]:
                return DeploymentResult(
                    ...
                    site_id=site_id,
                    site_name=actual_site_name,
                    error_message=f"Failed to deploy ZIP: {deploy_response.text}",
                    error_code="ZIP_DEPLOY_FAILED",
                    variant_id=variant_id,
                )
    ...
    def _deploy_with_requests(
        ...
        # Use path without leading slash for consistency with upload URL
        ...
        # NOTE: Per Netlify OpenAPI spec, uploadDeployFile does NOT require auth
        # The deploy_id itself acts as the authorization (it's a capability URL)
        if file_hash in required_files:
            upload_headers = {
                "Content-Type": "application/octet-stream",
            }

            # Use simple path without leading slash - Netlify adds it internally
            upload_response = requests.put(
                f"{self.netlify_api_base}/deploys/{deploy_id}/files/{file_path}",
